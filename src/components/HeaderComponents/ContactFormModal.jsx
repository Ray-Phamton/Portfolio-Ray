...
const ContactFormModal = ({ isOpen, onClose }) => {
  ...
  // If it's not open, it doesn't render anything (null)
  ...
  // Modal and Darkened Background
    ...
    >
      {/* Form Container */}
      <div
        className='relative w-full max-w-lg p-8 bg-zinc-800 rounded-xl shadow-2xl transform transition-all duration-300 scale-100 '
      ...
      >
        ...
        <form action={FORMSUBMIT_ENDPOINT} method='POST'>
          <div className='mb-4'>
            ...
            <input
              type='text' name='name' id='name' required className='text-gray-100 mt-1 block w-full px-4 py-2 border border-violet-600 rounded-lg focus:ring-violet-600 focus:border-violet-300
              focus:ring-2 outline-none' placeholder='Raymon Reyes'
            />
          </div>
          <div className='mb-4'>
            <label htmlFor='email' className='block text-sm font-medium text-gray-100'>Correo Electrónico</label>
            <input
              type='email' name='reply-to' id='email' required className='text-gray-100 mt-1 block w-full px-4 py-2 border border-violet-600 rounded-lg focus:ring-violet-600 focus:border-violet-300
              focus:ring-2 outline-none' placeholder='ejemplo@dominio.com'
            />
          </div>
          <div className='mb-6'>
